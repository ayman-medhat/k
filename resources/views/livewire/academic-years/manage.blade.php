...
<style>
    ...
    .badge-green {
        background: var(--crm-badge-green-bg); color: var(--crm-badge-green-text);
    }
    .btn-icon {
    ...
    <div class="header">
        <h1>{{ __('academic-years.page_title') }}</h1>
        <a href="{{ route('academic-years.create') }}" wire:navigate class="btn-primary">{{ __('academic-years.add_new') }}</a>
    </div>
    ...
    <div class="glass-panel">
        <table>
            <thead>
                <tr>
                    <th>{{ __('academic-years.year') }}</th>
                    <th>{{ __('academic-years.start_date') }}</th>
                    <th>{{ __('academic-years.end_date') }}</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($year->is_current)
                            <span class="badge badge-green">{{ __('academic-years.is_current') }}</span>
                        @else
                            <span class="badge">{{ __('general.inactive') }}</span>
                        @endif
                    </td>
                    <td>
                        <a href="{{ route('terms', ['academic_year_id' => $year->id]) }}" wire:navigate class="btn-secondary" style="padding: 0.25rem 0.75rem; font-size: 0.8rem;">
                            {{ $year->terms->count() }} {{ __('nav.terms') }}
                        </a>
                    </td>
                    <td>
                        @if(!$year->is_current)
                        <button wire:click="setCurrent({{ $year->id }})" class="btn-icon">Set Current</button>
                        @endif
                        <a href="{{ route('academic-years.edit', $year) }}" wire:navigate class="btn-icon" style="margin-left: 0.5rem;">{{ __('general.edit') }}</a>
                        <button wire:click="delete({{ $year->id }})" wire:confirm="{{ __('academic-years.delete_confirm') }}" class="btn-icon" style="color: #ef4444; margin-left: 0.5rem;">{{ __('general.delete') }}</button>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="empty-state">
                        <p>{{ __('academic-years.no_years') }}</p>
                        <p style="font-size: 0.875rem;">Create your first academic year to get started.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
