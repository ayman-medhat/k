...
<style>
    ...
    .subject-checklist { display: flex; flex-direction: column; gap: 0.25rem; }
    .subject-checklist label {
    ...
    .subject-checklist label:hover { background: var(--crm-input-bg); }
    ...
    <div class="form-card">
        <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.75rem;">
            <a href="{{ route('grades') }}" wire:navigate class="btn-secondary" style="padding: 0.4rem 0.8rem; font-size: 0.85rem;">{{ __('general.back') }}</a>
        </div>
        <h1>{{ $grade ? __('grades.edit_grade') : __('grades.new_grade') }}</h1>

        <form wire:submit.prevent="save">
            <div class="grid-2">
                <div class="form-group">
                    <label>{{ __('general.name_en') }}</label>
                    <input type="text" wire:model.blur="name" placeholder="Grade 1">
                    @error('name') <span class="error">{{ $message }}</span> @enderror
                </div>
                <div class="form-group">
                    <label>{{ __('general.name_ar') }}</label>
                    <input type="text" wire:model.blur="name_ar" placeholder="الصف الأول" dir="rtl">
            ...
            <div class="grid-2">
                <div class="form-group">
                    <label>{{ __('grades.level_order') }}</label>
                ...
                <div class="form-group">
                    ...
                    <input type="number" wire:model="num_classes" min="0" max="26" placeholder="3">
                    @error('num_classes') <span class="error">{{ $message }}</span> @enderror
                    <small style="color: var(--crm-text-muted); font-size: 0.75rem;">Auto-generates classes A–Z.</small>
                </div>
            </div>
            ...
            <hr class="divider">
            <div style="font-weight: 600; color: var(--crm-text-muted); margin-bottom: 0.75rem;">📚 Assign Subjects</div>
            <div class="subject-checklist">
                @foreach($this->rootSubjects as $subject)
                <label>
                    <input type="checkbox"
                        value="{{ $subject->id }}"
                        wire:model.live="assignedSubjectIds"
                        style="appearance: none; -webkit-appearance: none; width: 1.1rem; height: 1.1rem; border-radius: 0.2rem; border: 2px solid var(--crm-input-border); background: var(--crm-input-bg); cursor: pointer; flex-shrink: 0; display: inline-flex; align-items: center; justify-content: center;"
                    >
                    <span style="font-weight: 600;">{{ $subject->name }}</span>
                    @if($subject->is_religion_based)
                        <span style="font-size: 0.7rem; padding: 0.1rem 0.4rem; border-radius: 9999px; background: #fef3c7; color: #b45309;">Faith-based</span>
                    @endif
                    @if(!$subject->is_main)
                        <span style="font-size: 0.7rem; padding: 0.1rem 0.4rem; border-radius: 9999px; background: #fef3c7; color: #b45309;">Opt</span>
                    @endif
                </label>
                @if($subject->children->count())
                    ...
                    <label style="padding-left: 2rem;">
                        ...
                        @if($child->religion)
                            <span style="font-size: 0.7rem; color: var(--crm-text-muted);">({{ $child->religion }})</span>
                        @endif
                        @if(!$child->is_main)
                            <span style="font-size: 0.7rem; padding: 0.1rem 0.4rem; border-radius: 9999px; background: #fef3c7; color: #b45309;">Opt</span>
                        @endif
            ...
            <div class="actions">
                <a href="{{ route('grades') }}" wire:navigate class="btn-secondary">{{ __('general.cancel') }}</a>
