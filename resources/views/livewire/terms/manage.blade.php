...
<style>
    ...
    <div class="header">
        ...
        <div style="display: flex; align-items: center; gap: 1rem;">
            ...
            <a href="{{ route('terms.create') }}" wire:navigate class="btn-primary">{{ __('terms.add_new') }}</a>
        </div>
    </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $term->academicYear?->name ?? __('general.na') }}</td>
                    ...
                    <td>
                        @if(!$term->is_current)
                        <button wire:click="setCurrent({{ $term->id }})" class="btn-icon">Set Current</button>
                        @endif
                        <a href="{{ route('terms.edit', $term) }}" wire:navigate class="btn-icon" style="margin-left: 0.5rem;">{{ __('general.edit') }}</a>
                        <button wire:click="delete({{ $term->id }})" wire:confirm="{{ __('terms.delete_confirm') }}" class="btn-icon" style="color: #ef4444; margin-left: 0.5rem;">{{ __('general.delete') }}</button>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="empty-state">
                        <p>{{ __('terms.no_terms') }}</p>
                        <p style="font-size: 0.875rem;">Create your first term to get started.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
