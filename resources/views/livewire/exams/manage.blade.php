...
<style>
    ...
    <div class="header">
        <h1>{{ __('exams.page_title') }}</h1>
        <div style="display: flex; align-items: center; gap: 1rem;">
            <select wire:model.live="grade_id" class="filter-select">
                <option value="">{{ __('general.all') }} {{ __('nav.grades') }}</option>
                @foreach($this->grades as $g)
                    <option value="{{ $g->id }}">{{ $g->name }}</option>
                @endforeach
            </select>
            <select wire:model.live="term_id" class="filter-select">
                <option value="">{{ __('general.all') }} {{ __('nav.terms') }}</option>
                @foreach($this->terms as $t)
                    <option value="{{ $t->id }}">{{ $t->name }} ({{ $t->academicYear?->name ?? '' }})</option>
                @endforeach
            </select>
            <a href="{{ route('exams.create') }}" wire:navigate class="btn-primary">{{ __('exams.add_new') }}</a>
        </div>
    </div>
    ...
    <div class="glass-panel">
        <table>
            <thead>
                <tr>
                    <th>{{ __('exams.exam_name') }}</th>
                    <th>{{ __('general.grade') }}</th>
                    <th>{{ __('exams.term') }}</th>
                    <th>{{ __('exams.date') }}</th>
                    <th>{{ __('nav.subjects') }}</th>
                    <th>Records</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $exam->grade?->name ?? __('general.na') }}</td>
                    <td>{{ $exam->term?->name ?? __('general.na') }}</td>
                    <td>{{ $exam->date->format('M d, Y') }}</td>
                    <td>
                        <span class="badge">{{ $exam->subjects->count() }}</span>
                    </td>
                    <td>
                        <span class="badge badge-green">{{ $exam->records->count() }}</span>
                    </td>
                    <td>
                        <a href="{{ route('exams.marks', $exam) }}" wire:navigate class="btn-icon" style="color: #059669;">{{ __('exams.enter_marks') }}</a>
                        <a href="{{ route('exams.edit', $exam) }}" wire:navigate class="btn-icon" style="margin-left: 0.5rem;">{{ __('general.edit') }}</a>
                        <button wire:click="delete({{ $exam->id }})" wire:confirm="{{ __('exams.delete_confirm') }}" class="btn-icon" style="color: #ef4444; margin-left: 0.5rem;">{{ __('general.delete') }}</button>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" class="empty-state">
                        ...
                        <p style="font-size: 0.875rem;">Create your first exam to get started.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
