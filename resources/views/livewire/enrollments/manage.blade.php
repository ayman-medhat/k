...
<style>
    :root {
        ...
        --crm-badge-green-bg: #d1fae5;
        --crm-badge-green-text: #065f46;
        --crm-badge-yellow-bg: #fef3c7;
        --crm-badge-yellow-text: #92400e;
        --crm-badge-red-bg: #fee2e2;
        --crm-badge-red-text: #991b1b;
        --crm-badge-blue-bg: #dbeafe;
        --crm-badge-blue-text: #1e40af;
    }
    .dark {
        ...
        --crm-badge-green-bg: #064e3b;
        --crm-badge-green-text: #6ee7b7;
        --crm-badge-yellow-bg: #78350f;
        --crm-badge-yellow-text: #fcd34d;
        --crm-badge-red-bg: #7f1d1d;
        --crm-badge-red-text: #fca5a5;
        --crm-badge-blue-bg: #1e3a5f;
        --crm-badge-blue-text: #93c5fd;
    }
    .crm-container {
    ...
    <div class="header">
        <h1>{{ __('enrollments.page_title') }}</h1>
        <div class="filters">
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="{{ __('general.search') }}" class="search-box">
            <select wire:model.live="academic_year_id" class="filter-select">
            ...
            <a href="{{ route('enrollments.create') }}" wire:navigate class="btn-primary">{{ __('enrollments.new_enrollment') }}</a>
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr>
                    <td style="font-weight: 600;">
                        <a href="{{ route('students.edit', $enrollment->student) }}" wire:navigate style="color: var(--crm-text); text-decoration: none;">
                            {{ $enrollment->student?->contact?->nameEn ?? __('general.na') }}
                        </a>
                    </td>
                    <td>{{ $enrollment->academicYear?->name ?? __('general.na') }}</td>
                    <td>{{ $enrollment->grade?->name ?? __('general.na') }}</td>
                    <td>{{ $enrollment->section?->name ?? __('general.no_data') }}</td>
                    ...
                    <td>
                        <a href="{{ route('enrollments.edit', $enrollment) }}" wire:navigate class="btn-icon">{{ __('general.edit') }}</a>
                        <button wire:click="delete({{ $enrollment->id }})" wire:confirm="{{ __('enrollments.delete_confirm') }}" class="btn-icon" style="color: #ef4444; margin-left: 0.5rem;">{{ __('general.delete') }}</button>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="7" class="empty-state">
                        <p>{{ __('enrollments.no_enrollments') }}</p>
                        <p style="font-size: 0.875rem;">Enroll a student to get started.</p>
                    </td>
                </tr>
                @endforelse
            </tbody>
