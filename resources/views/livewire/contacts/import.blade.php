...
<style>
    ...
    <div class="header">
        <h1>{{ __('contacts.import_title') }}</h1>
        <a href="{{ route('contacts') }}" wire:navigate class="btn-secondary">{{ __('contacts.back_to_contacts') }}</a>
    </div>

    <div class="glass-panel" style="max-width: 42rem; margin: 0 auto;">
        @if($results)
            <h2 style="font-size: 1.25rem; font-weight: 700; color: var(--crm-text); margin-bottom: 0.75rem;">{{ __('contacts.import_complete') }}</h2>

            <div class="stat-grid">
                <div class="stat-card success">
                    <div class="stat-value">{{ $results['imported'] }}</div>
                    <div class="stat-label">{{ __('contacts.imported') }}</div>
                </div>
                <div class="stat-card success">
                    <div class="stat-value">{{ $results['updated'] }}</div>
                    <div class="stat-label">{{ __('contacts.updated') }}</div>
                </div>
                <div class="stat-card {{ $results['skipped'] > 0 ? 'warning' : 'success' }}">
                    <div class="stat-value">{{ $results['skipped'] }}</div>
                    <div class="stat-label">{{ __('contacts.skipped') }}</div>
                </div>
            </div>

            @if(count($results['errors']) > 0)
                <div class="error-list">
                    <h3 style="font-size: 0.9rem; font-weight: 600; color: #ef4444; margin-bottom: 0.75rem;">{{ __('contacts.errors') }} ({{ count($results['errors']) }})</h3>
            ...
            <div style="margin-top: 0.75rem; display: flex; gap: 0.75rem;">
                <button wire:click="$set('results', null)" class="btn-primary">{{ __('contacts.import_another') }}</button>
                <a href="{{ route('contacts') }}" wire:navigate class="btn-secondary">{{ __('contacts.view_contacts') }}</a>
            </div>
        @else
            <form wire:submit="import">
                <div class="drop-zone {{ $file ? 'has-file' : '' }}"
                    ...
                    @else
                        <div style="font-weight: 600; color: var(--crm-text);">{{ __('contacts.drop_file') }}</div>
                        <div style="font-size: 0.8rem; color: var(--crm-text-muted); margin-top: 0.75rem;">{{ __('contacts.file_support') }}</div>
                    @endif
                ...
                <div class="hint">
                    <strong>{{ __('contacts.expected_columns') }}</strong>
                    <code>nameEn</code> <code>nameAr</code> <code>email</code> <code>phone</code>
                    <code>national_id</code> <code>categories</code>
                    — <a href="{{ route('contacts.import.template') }}" style="color: var(--crm-input-focus-border);">{{ __('contacts.download_template') }}</a>
                </div>

                <div style="margin-top: 0.75rem; text-align: right;">
                    <button type="submit" class="btn-primary" wire:loading.attr="disabled" wire:target="file">
                        <span wire:loading.remove wire:target="import">{{ __('contacts.import_title') }}</span>
                        <span wire:loading wire:target="import"><span class="spinner"></span> {{ __('contacts.importing') }}</span>
                    </button>
                </div>
            </form>
        @endif
    ...
    <div class="modal-overlay" wire:ignore.self x-data x-init="$el.style.display='flex'">
        <div class="modal-box" style="background: var(--crm-panel-bg); backdrop-filter: blur(10px); border-radius: 1rem; border: 1px solid var(--crm-panel-border); padding: 2rem; max-width: 500px; width: 100%; margin: 2rem auto; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25);">
            <h3 style="font-size: 1.25rem; font-weight: 700; color: var(--crm-text); margin: 0 0 0.5rem;">{{ __('contacts.duplicate_records') }}</h3>
            <p style="color: var(--crm-text-muted); font-size: 0.9rem; margin-bottom: 0.75rem;">
                {{ count($duplicates) }} {{ __('contacts.records_exist') }}
            </p>
            <div style="max-height: 200px; overflow-y: auto; margin-bottom: 0.75rem; border: 1px solid var(--crm-divider); border-radius: 0.5rem; padding: 0.5rem;">
                ...
                <div style="display: flex; justify-content: space-between; padding: 0.35rem 0; border-bottom: 1px solid var(--crm-divider-dashed); font-size: 0.85rem; color: var(--crm-text);">
                    ...
                    <span style="color: var(--crm-text-muted);">{{ __('contacts.id') }} {{ $dup['existing']['national_id'] ?? __('general.na') }}</span>
                </div>
                @endforeach
            </div>
            <p style="color: var(--crm-text-muted); font-size: 0.85rem; margin-bottom: 0.75rem;">{{ __('general.what_to_do') }}</p>
            <div style="display: flex; gap: 0.75rem; justify-content: flex-end;">
                <button wire:click="cancelImport" class="btn-secondary" style="padding: 0.6rem 1.2rem; font-size: 0.85rem;">{{ __('contacts.cancel_import') }}</button>
                <button wire:click="confirmImport('skip')" class="btn-primary" style="background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); box-shadow: 0 4px 6px -1px rgba(245,158,11,0.4); padding: 0.6rem 1.2rem; font-size: 0.85rem;">{{ __('contacts.skip_duplicates') }}</button>
